...
interface CrawlingData {
  ...
  // 인덱스 시그니처: 동적으로 결정되는 컬럼을 수용
  // key는 컬럼 이름, value는 해당 데이터 값 (string 또는 number)
  [key: string]: any; //value속성은 any대신 string | number로 엄격관리 가능
}
...
const useCrawling = (): UseCrawlingResult => {
  ...
  const handleWebSocketMessage = useCallback(
    (event: MessageEvent) => {
      try {
        const data: CrawlingData = JSON.parse(event.data); //웹소켓메세지event.data를 JSON객체로 변환
    ...
    [setMessages, setProgress, setIsCrawling, setShowCompletionMessage]
  ); // 상태 setter 함수들을 의존성 배열에 포함
