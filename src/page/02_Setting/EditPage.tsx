...
import { getRobots, getPreview2, updateSetting, getCondtions } from '../../API/02_SettingApi';
...
export default function RegPage() {
    ...
    const getCondtionsData = async () => {
      ...
      const result = data.map((row: ConditionTableRows, i: number) => ({
        ...
        setNewData({
          ...
          pagingType: row?.pagingType,
          pagingArea: row?.pagingArea,
          pagingNextbtn: row?.pagingNextbtn,
    ...
    const handleRobots = async () => {
        ...
        try {
          ...
          const resPreview = await getPreview2(newData.url)
    ...
    const handleDetailLoad = async () => {
      if(!detailUrl && row.linkArea) {
        setAlertMsg('상세영역을 다시 지정해주세요.')
        setOpenErrorAlert(true)
        setLoading(false)
        return;
      }
      ...
      try {
        const resPreview = await getPreview2(detailUrl) 
