...
export interface CrawlingSchedule {
  ...
  userId: string; // 작업을 등록한 사용자 ID (백엔드에서 설정)
 ...
 */
export const addCrawlingSchedule = async (
  cronExpression: string
): Promise<CrawlingSchedule> => {
  const response = await httpClient.post<CrawlingSchedule>(
    "/schedules/crawling",
    {
      cronExpression,
    }
...
/**
 * 현재 인증된 사용자의 크롤링 스케줄 작업 목록을 조회합니다.
 * 사용자 ID는 백엔드에서 JWT를 통해 자동으로 추출됩니다.
 *
...
export const getCrawlingSchedules = async (): Promise<CrawlingSchedule[]> => {
  const response = await httpClient.get<CrawlingSchedule[]>(
    "/schedules/crawling"
  );
  return response.data;
};
 ...
 */
export const updateCrawlingSchedule = async (
  id: number,
  cronExpression: string
): Promise<CrawlingSchedule> => {
  const response = await httpClient.put<CrawlingSchedule>(
    `/schedules/crawling/${id}`,
    {
      cronExpression,
    }
  );
