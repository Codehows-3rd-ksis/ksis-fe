import AxiosInstance from "./AxiosInstance"; // HTTP 요청을 위한 AxiosInstance를 임포트합니다.
...
export const startCrawling = async (): Promise<void> => {
  try {
    // `AxiosInstance.post`를 사용하여 백엔드의 크롤링 시작 엔드포인트에 POST 요청을 보냅니다.
    // 백엔드에서 JWT를 통해 사용자 ID를 추출하므로, 클라이언트에서 userId를 명시적으로 전달할 필요가 없습니다.
    await AxiosInstance.post(`/crawling/start`);
